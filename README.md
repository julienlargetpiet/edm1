...
+-----------------------------------+-----------------------------------+
| `pu`{#cost_and_taxes_:_pu}        | is the price of a single elements |
|                                   | without taxes                     |
+-----------------------------------+-----------------------------------+
| `pr                               | is the duty-free price of the     |
| ix_ht`{#cost_and_taxes_:_prix_ht} | whole set of elements             |
+-----------------------------------+-----------------------------------+
| `tva`{#cost_and_taxes_:_tva}      | is the percentage of all taxes    |
+-----------------------------------+-----------------------------------+
| `prix                             | is the price of all the elements  |
| _ttc`{#cost_and_taxes_:_prix_ttc} | with taxes                        |
+-----------------------------------+-----------------------------------+
| `prix                             | is the cost of all the taxes      |
| _tva`{#cost_and_taxes_:_prix_tva} |                                   |
+-----------------------------------+-----------------------------------+
| `                                 | is the price of a single element  |
| pu_ttc`{#cost_and_taxes_:_pu_ttc} | taxes included                    |
+-----------------------------------+-----------------------------------+
| `                                 | is the discount percentage        |
| adjust`{#cost_and_taxes_:_adjust} |                                   |
+-----------------------------------+-----------------------------------+
| `prix_d                           | is the free-duty price of an      |
| _ht`{#cost_and_taxes_:_prix_d_ht} | element after discount            |
+-----------------------------------+-----------------------------------+
| `prix_d_t                         | is the price with taxes of an     |
| tc`{#cost_and_taxes_:_prix_d_ttc} | element after discount            |
+-----------------------------------+-----------------------------------+
| `pu_d`{#cost_and_taxes_:_pu_d}    | is the price of a single element  |
|                                   | after discount and without taxes  |
+-----------------------------------+-----------------------------------+
| `pu_d                             | is the free-duty price of a       |
| _ttc`{#cost_and_taxes_:_pu_d_ttc} | single element after discount     |
|                                   |                                   |
|                                   | the function return a vector with |
|                                   | the previous variables in the     |
|                                   | same order those that could not   |
|                                   | be calculated will be represented |
|                                   | with NA value                     |
+-----------------------------------+-----------------------------------+
:::
::: container
...
## date_sort Allow to ascendely or desendely sort dates in a vector. {#date_sort}

### Description

date_sort Allow to ascendely or desendely sort dates in a vector.
...
### Arguments
  ...
  `v_old_begin`{#diff_xlsx_:_v_old_begin}     is the corrdinate (row, column) where the data to be compared starts
  ...
  `v_new_end`{#diff_xlsx_:_v_new_end}         is the same but for its end
...
### Arguments
  ...
  `df_ins`{#insert_df_:_df_ins}     is the dataset to be inserted ins_loc is a vector containg two parameters (row, column) of the begining for the insertion
...
Allow to create patterns which have a part that is varying aech time
randomly
...
index) according to these rules:
...
First case: Search for patterns strictly, it means that the searched
pattern(s) will be matched only if the patterns containded in the vector
that is beeing explored by the function are present like this
c(\"pattern_searched\", \"other\", \..., \"pattern_searched\") and not
as c(\"other_thing pattern_searched other_thing\", \"other\", \...,
\"pattern_searched other_thing\") Second case: It is the opposite to the
first case, it means that if the pattern is partially present like in
the first position and the last, it will be considered like a matched
pattern
...
### Arguments
  ...
  `condition_l`{#see_df_:_condition_l}       is the vector of the possible conditions (\"==\", \"\>\", \"\<\", \"!=\", \"%\")
  `val_l`{#see_df_:_val_l}                   is a list with the vector of values related to condition_l (so the values has to be placed in the same order)
...
+-----------------------------------+-----------------------------------+
| `sep_`{#see_inside_:_sep_}        | is a vector containing the        |
|                                   | separator for each csv type file  |
|                                   | in order following the operating  |
|                                   | system file order, if the vector  |
|                                   | does not match the number of the  |
|                                   | csv files found, it will assume   |
|                                   | the separator for the rest of the |
|                                   | files is the same as the last csv |
|                                   | file found. It means that if you  |
|                                   | know the separator is the same    |
|                                   | for all the csv type files, you   |
|                                   | just have to put the separator    |
|                                   | once in the vector.               |
+-----------------------------------+-----------------------------------+
| `uniq                             | is a pattern that you know will   |
| ue_sep`{#see_inside_:_unique_sep} | never be in your input files      |
+-----------------------------------+-----------------------------------+
| `rec`{#see_inside_:_rec}          | alloaw to get files recursively   |
|                                   |                                   |
|                                   | If x is the return value, to see  |
|                                   | all the files name, position of   |
|                                   | the columns and possible sheet    |
|                                   | name associanted with, do the     |
|                                   | following:                        |
|                                   | print(x\[(grep(unique_sep,        |
|                                   | x)1+1):length(x)\]) If you just   |
|                                   | want to see the columns do the    |
|                                   | following:                        |
|                                   | print(x1:(grep(unique_sep, x) -   |
|                                   | 1))                               |
+-----------------------------------+-----------------------------------+
| `pattern`{#see_inside_:_pattern}  | is a vector containin the file    |
|                                   | extension of the spreadsheets     |
|                                   | (\"xlsx\", \"csv\"\...)           |
+-----------------------------------+-----------------------------------+
| `path`{#see_inside_:_path}        | is the path where are located the |
|                                   | files                             |
+-----------------------------------+-----------------------------------+
...
ex: if i want my vector c(1, 2) to be 5 of length this function will
return me: c(1, 2, 1, 2, 1)
...
the vector c(4, 1) with the coefficient 1 and the start position at the
second column is containded in the dataframe
...
### Arguments
  ...
  `spe_l`{#pattern_tuning_:_spe_l}                 is the source vector from which the new characters will be replace old ones
...
### Arguments
  ...
  ----------------------- -----------------------------------------------------------------------------------------------------------------
  `vec`{#match_n_:_vec}   is th einput vector mc is a vector containing the values you want to get the index for the nth occurence in vec
